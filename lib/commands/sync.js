...
module.exports = async function (argv) {
  ...
  let getCursors = function (collection) {
    return Promise.all([
      ...
      `, { ttl: 1800, batchSize: 1000, options: { stream: true } }),
      ta.q(aql`
  ...
  let sourceDBExists = await sa.exists()
  ...
  confMessage += `  Collections and indexes that don't exist on source will be ${colors.red('deleted')} from target!!! \n`
  ...
  await Promise.map(targetCollectionList, async function (collection) {
    if (!sourceCollectionList.includes(collection)) {
      console.log(`Collection '${colors.yellow(collection)}' does not exist on source, deleting...`)
  ...
  await Promise.map(collectionsToSync, async function (collection) {
    console.log(`${colors.yellow(collection)} - Synchronizing indexes ======================================`)
    let sColl = sa.collection(collection)
    let tColl = ta.collection(collection)
    ...
    for (let tIndex of targetIndexes) {
      let hasIndex = !!sourceIndexes.find(function (sIndex) {
        return (
          tIndex.name === sIndex.name &&
      ...
      if (hasIndex === false) {
        console.log(`Index '${colors.yellow(`${tIndex.name} - ${tIndex.fields}`)}' does not exist on source '${colors.yellow(collection)}' collection, deleting...`)
        await tColl.dropIndex(tIndex.id)
      }
    }

    targetIndexes = (await tColl.indexes()).filter(idx => idx.type !== 'primary')
    ...
    for (let sIndex of sourceIndexes) {
      ...
      if (hasIndex === false) {
        console.log(`Index '${colors.yellow(`${sIndex.name} - ${sIndex.fields}`)}' does not exist on target '${colors.yellow(collection)}' collection, creating...`)
        await tColl.createIndex(_.pick(sIndex, 'name', 'type', 'fields', 'unique', 'sparse', 'deduplicate'))
      }
    }
  }, { concurrency: 99 })
